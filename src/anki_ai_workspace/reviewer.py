...
#anki-ai-workspace-shortcuts{position:fixed;left:64px;bottom:20px;z-index:1000;display:flex;max-width:calc(100vw - 84px);height:34px;gap:6px;margin:0;padding:0;overflow-x:auto;overflow-y:hidden;background:transparent!important;scrollbar-width:none}#anki-ai-workspace-shortcuts::-webkit-scrollbar{display:none}#anki-ai-workspace-shortcuts[hidden]{display:none!important}#anki-ai-workspace-shortcuts button{flex:0 0 auto;height:34px;margin:0;padding:0 13px;border:1px solid rgba(0,0,0,.12)!important;border-radius:17px;background:#fff!important;color:#171717!important;font-size:12px!important;font-weight:600!important;line-height:32px!important;white-space:nowrap;cursor:pointer;box-shadow:0 3px 10px rgba(0,0,0,.12);transition:transform .15s ease,box-shadow .15s ease,opacity .15s ease}#anki-ai-workspace-shortcuts button:hover{transform:translateY(-1px);box-shadow:0 5px 13px rgba(0,0,0,.16)}#anki-ai-workspace-shortcuts button:disabled{opacity:.48;cursor:default;transform:none;box-shadow:0 3px 10px rgba(0,0,0,.08)}
...
#anki-ai-workspace-menu-profile,.anki-ai-workspace-group{padding:8px 10px 5px;color:#777;font-size:10px!important;font-weight:700!important;letter-spacing:.08em!important;text-transform:uppercase!important}#anki-ai-workspace-menu button,#anki-ai-workspace-selector button{display:flex;width:100%;min-height:38px;align-items:center;margin:0;padding:0 10px;border:0!important;border-radius:7px;background:transparent!important;color:inherit!important;font-size:13px!important;font-weight:500!important;text-align:left;cursor:pointer}#anki-ai-workspace-menu button:hover,#anki-ai-workspace-selector button:hover{background:#f2f2f2!important}.anki-ai-workspace-menu-divider{height:1px;margin:7px 4px;background:#e8e8e8}.anki-ai-workspace-empty{padding:8px 10px;color:#777;font-size:12px!important}.anki-ai-workspace-menu .anki-ai-workspace-configure{font-size:12px!important;font-weight:650!important}
...
(()=>{
 ...
 window.AnkiAIWorkspace.render=data=>{menu.hidden=!data.menu_open;heading.textContent=data.menu.profile_name||'AI Workspace';actions.replaceChildren();data.menu.actions.forEach(action=>actions.append(button(action.title,()=>send({action:'select_action',action_id:action.id}))));if(!data.menu.has_profile){const empty=document.createElement('div');empty.className='anki-ai-workspace-empty';empty.textContent='No deck profile assigned.';actions.append(empty);actions.append(button('Configure profiles…',()=>send({action:'configure_profiles'}),'anki-ai-workspace-configure'))}const divider=document.createElement('div');divider.className='anki-ai-workspace-menu-divider';actions.append(divider);actions.append(button('Custom chat',()=>send({action:'open_custom'})));actions.append(button(data.menu.deck_general_label,()=>send({action:'open_deck_general'})));base(data);requestAnimationFrame(placeMenu)};
